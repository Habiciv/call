...
function cleanAvatar(value:unknown){
 ...
 if(avatar.length>24000||!AVATAR.test(avatar))throw new Error('AVATAR_INVALID');
 return avatar;
}
function peerQuery(){
...
export async function POST(req:Request){
 try{
  ...
  if(action==='join'){
   ...
   if(action==='signal'){
    if(!ID.test(target||'')||target===id||!data||JSON.stringify(data).length>32000)return Response.json({error:'Sinal inválido'},{status:400});
    await db.prepare('UPDATE peers SET seen=? WHERE id=?').bind(now,id).run();
    await db.prepare('INSERT INTO signals(room,sender,target,data,created) SELECT ?,?,?,?,? WHERE EXISTS (SELECT 1 FROM peers WHERE id=? AND room=? AND seen>?)')
     .bind(room,id,target,JSON.stringify(data),now,target,room,now-30000).run();
    return Response.json({ok:true});
   }
   if(action==='avatar'){
    if(!ID.test(target||''))return Response.json({error:'Perfil inválido'},{status:400});
    await db.prepare('UPDATE peers SET seen=? WHERE id=?').bind(now,id).run();
    ...
    if(!profile)return Response.json({error:'Pessoa não encontrada na sala.'},{status:404});
