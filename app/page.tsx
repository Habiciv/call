...
function Media({stream,local=false,forceVideo=false,volume=100,masterVolume=100,audioScale=100,sensitivity='normal',onSpeaking}:{stream:MediaStream;local?:boolean;forceVideo?:boolean;volume?:number;masterVolume?:number;audioScale?:number;sensitivity?:Sensitivity;onSpeaking?:(value:boolean)=>void}){
 ...
 useEffect(()=>{if(local)return;const retry=()=>{if(audioRef.current)void audioRef.current.play().then(()=>setBlocked(false)).catch(()=>{})};window.addEventListener('pointerdown',retry,{passive:true});return()=>window.removeEventListener('pointerdown',retry)},[local]);
 return <><audio ref={audioRef} className="remote-audio" autoPlay={!local} preload="auto"/>{onSpeaking&&<SpeakingSensor stream={stream} enabled={!local} sensitivity={sensitivity} onChange={onSpeaking}/>} {hasVideo&&<video ref={videoRef} autoPlay playsInline muted controls className="screen-video"/>}{blocked&&<button type="button" className="enable-audio" onClick={enableAudio}><Volume2 size={15}/> Ativar áudio</button>}</>;
}
...
const peerStateText={
 new:'Preparando áudio…',
 connecting:'Conectando áudio…',
 connected:'Áudio conectado',
 reconnecting:'Reconectando áudio…',
 failed:'Falha no áudio',
} as const;
